...
print(f"on_file_upload event: {event_id}")
...
while not q.empty():
    ...
    if data.startswith("data:"):
        ...
        for i, v in enumerate(result):
            vstr = str(v)[:80].encode('ascii', 'replace').decode('ascii')
            print(f"  [{i}]: {vstr}")
...
train_inputs = [
    ...
    "",             # requirement
    "10",           # n_future_val
    "\u5206\u949f",  # n_future_unit (minutes in Chinese)
    "",             # chart_requirement
]

r4 = requests.post(f"{API}/gradio_api/call/on_train",
                   json={"data": train_inputs}, timeout=120)
print(f"on_train HTTP: {r4.status_code}")
print(f"on_train body: {r4.text[:300]}")
