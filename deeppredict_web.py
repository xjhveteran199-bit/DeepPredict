...
class SklearnPredictor:
    ...
    def train(self, X, y, task_type, model_name, params):
        try:
            ...
            n_estimators = params.get('n_estimators', 50) if hasattr(self.model, 'n_estimators') else 1
            max_show = min(10, n_estimators) if n_estimators > 1 else 1
            ...
            if task_type == 'classification':
                ...
                if max_show == 1:
                    ...
                    self.train_history['accuracy'] = [round(acc, 6)]
                else:
                    ...
                    for ep in range(max_show):
                        ...
                        train_pred = self.model.predict(X_train)
                        acc = accuracy_score(y_train, train_pred)
                        self.train_history['accuracy'].append(round(acc, 6))
            else:
                from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score
                if max_show == 1:
                    self.model.fit(X_train, y_train)
                    train_pred = self.model.predict(X_train)
                    loss_val = np.sqrt(mean_squared_error(y_train, train_pred))
                    self.train_history['loss'] = [round(loss_val, 6)]
                else:
                    ...
                    for ep in range(max_show):
                        ...
                        train_pred = self.model.predict(X_train)
                        loss_val = np.sqrt(mean_squared_error(y_train, train_pred))
                        self.train_history['loss'].append(round(loss_val, 6))
        ...
        if success and lstm_pred and lstm_pred.is_fitted:
            try:
                ...
                if future_preds is not None and len(future_preds) > 0:
                    ...
                    hist_png_path = None
                    if predictor and hasattr(predictor, 'train_history') and predictor.train_history:
