...
class PatchTSTPredictor:
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                with torch.no_grad():
                    ...
                    val_pred_denorm = val_pred_np * self._target_std + self._target_mean
                    y_test_np = y_test.numpy() * self._target_std + self._target_mean
                    val_mse = float(np.mean((val_pred_denorm - y_test_np) ** 2))
                    val_mae = float(mean_absolute_error(y_test_np.flatten(), val_pred_denorm.flatten()))
                    ss_res = np.sum((y_test_np - val_pred_denorm) ** 2)
                    ss_tot = np.sum((y_test_np - np.mean(y_test_np)) ** 2)
                    val_r2 = float(1 - ss_res / (ss_tot + 1e-8))

                    # 训练集指标
                    train_pred = self.model(X_train.to(self.device))
                    train_pred_np = train_pred.cpu().numpy()
                    train_pred_denorm = train_pred_np * self._target_std + self._target_mean
                    y_train_np = y_train.numpy() * self._target_std + self._target_mean
                    train_mse = float(np.mean((train_pred_denorm - y_train_np) ** 2))
                    train_mae = float(mean_absolute_error(y_train_np.flatten(), train_pred_denorm.flatten()))
                    ss_res_tr = np.sum((y_train_np - train_pred_denorm) ** 2)
                    ss_tot_tr = np.sum((y_train_np - np.mean(y_train_np)) ** 2)
                ...
                if (epoch + 1) % 5 == 0 or epoch == 0:
                    logger.info(f"PatchTST Epoch {epoch+1}/{epochs}, Loss: {avg_loss:.6f}, ValMSE: {val_mse:.6f}, ValR2: {val_r2:.4f}")
    ...
    def predict_future(self, X: np.ndarray, steps: int = 1) -> np.ndarray:
        ...
        if x_cur.ndim == 1:
            x_cur = x_cur.reshape(-1, 1)  # (n, 1) - 单变量模式
        n_features = x_cur.shape[1]
        ...
        while remaining > 0:
            ...
            if remaining > this_pred:
                # 更新 x_cur:移除前 this_pred 个,加入 this_pred 个预测
                # preds 已经是 1D (steps,);对于多变量,用第一个目标的预测更新第一列
                if n_features == 1:
                    # 单变量:直接拼接
                    x_cur = np.concatenate([x_cur[this_pred:], preds.reshape(-1, 1)], axis=0)
                else:
                    # 多变量:只更新第一列(目标列),其余列用0或最后值填充
                    tail = x_cur[this_pred:].copy()
                    tail[-this_pred:, 0] = preds
                    x_cur = tail
