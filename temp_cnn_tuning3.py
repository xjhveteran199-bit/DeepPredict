"""
CNN1D 参数快速调优 - 基于 CNN1DModelV4 (headless版本)
"""
import matplotlib
matplotlib.use('Agg')
...
def test_config(seq_len, epochs, hidden_channels, kernel_size):
    try:
        ...
        if success:
            ...
            print(f"  seq={seq_len}, ep={epochs}, hid={hidden_channels}, ker={kernel_size} => R2={r2:.4f}, RMSE={rmse:.4f}")
            ...
            print(f"  seq={seq_len}, ep={epochs}, hid={hidden_channels}, ker={kernel_size} => FAILED")
...
print("\n=== 阶段1: seq_len=365, epochs=150 ===")
for hidden in [64, 128]:
    for kernel in [5, 7]:
        r = test_config(seq_len=365, epochs=150, hidden_channels=hidden, kernel_size=kernel)
        results.append(r)

best = max([r for r in results if r['success']], key=lambda x: x['R2'], default=None)
print(f"阶段1 最佳: {best}")

# 阶段2: seq_len=180, epochs=150
print("\n=== 阶段2: seq_len=180, epochs=150 ===")
for hidden in [64, 128]:
    for kernel in [5, 7]:
...
if successful:
    best_all = max(successful, key=lambda x: x['R2'])
    print("\n" + "="*70)
    print("最优配置:")
    print(f"  seq_len={best_all['seq_len']}, epochs={best_all['epochs']}, hidden={best_all['hidden_channels']}, kernel={best_all['kernel_size']}")
    print(f"  R2={best_all['R2']:.4f}, RMSE={best_all['RMSE']:.4f}")
    print("="*70)
    
    with open(r"C:\Users\XJH\DeepPredict\cnn1d_tuning_results.json", 'w', encoding='utf-8') as f:
        json.dump({'best': best_all, 'all_results': results}, f, ensure_ascii=False, indent=2)
    print(f"结果已保存到 cnn1d_tuning_results.json")
